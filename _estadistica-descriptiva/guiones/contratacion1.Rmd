---
title: "Contratación en Antioquia durante diciembre de 2017"
author: "Kenneth Roy Cabrera Torres"
date: "Martes, 6 de marzo de 2018"
output: 
  html_document: 
    highlight: tango
    toc: yes
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```


## Adecuación de la base de datos.

```{r}
# Lee la base de datos.
contrata <- read.csv("Contratacion_Diciembre_2017.csv")

# Convierte a formato fecha los datos de fecha.
contrata$FECHA.INICIO <- as.POSIXct(strptime(contrata$FECHA.INICIO,
                                             format = "%m/%d/%Y %H:%M:%S"))
contrata$FECHA.SUSCRIPCIÓN <- as.POSIXct(strptime(contrata$FECHA.SUSCRIPCIÓN,
                                             format = "%m/%d/%Y %H:%M:%S"))

library(dplyr)
contrata <- contrata %>% 
            mutate(VALOR.CONTRATO = 
                as.numeric(as.character(
                    recode(VALOR.CONTRATO, `#############` = ""))),
                lapso    = as.numeric(FECHA.INICIO - FECHA.SUSCRIPCIÓN)/(3600*24),
                difPresup = DISPONIBILIDADES.PRESUPUESTALES -
                            REGISTROS.PRESUPUESTALES)

```


## Resumen inicial de la base de datos.

### Estructura

```{r}
# Muestra la estructura de la base de datos.
str(contrata)
```


### Resumen estadístico

```{r}
# Resumen de la base de datos.
summary(contrata)
```

## Manejo básico de base de datos.

El paquete `dplry` tiene los siguientes "verbos"

 - `select()` Seleccionar columnas (también permite reordenar).
 - `filtrar()` filtrar por filas.
 - `arrange()` ordenar o reordenar las filas.
 - `mutate()` Crear o modificar columnas.
 - `summarise()` Resumir valores.
 - `group_by` Permite agrupaciones en la estrategia de dividir y aplicar a cada grupo.
 
 
### Seleccionar

```{r}
library(dplyr)
select(contrata, CÓDIGO.CONTRATO, PLAZO.ESTIMADO, VALOR.CONTRATO)
```

```{r}
library(dplyr)
contrata1 <- select(contrata, CÓDIGO.CONTRATO, PLAZO.ESTIMADO, VALOR.CONTRATO)
head(contrata1)
```

```{r}
library(dplyr)
contrata2 <- contrata %>% 
            select(CÓDIGO.CONTRATO, PLAZO.ESTIMADO, VALOR.CONTRATO)
head(contrata2)
```


```{r}
library(dplyr)
contrata3 <- contrata %>%
             select(CÓDIGO.CONTRATO, CÓDIGO.DEL.PROYECTO:VALOR.EJECUTADO.DEL.PROYECTO)
head(contrata3)
```

```{r}
rm(contrata1, contrata2, contrata3)
```


### Filtar

```{r}
library(dplyr)
contrata1 <- contrata %>%
            filter(VALOR.CONTRATO > 50000000)
head(contrata1)
```

```{r}
library(dplyr)
contrata2 <- contrata %>%
            filter(VALOR.CONTRATO > 50000000 & PLAZO.ESTIMADO >= 60)
head(contrata2)
```

### Ordenar

```{r}
library(dplyr)
contrata3 <- contrata %>%
             arrange(desc(VALOR.CONTRATO))
head(contrata3)
```


### Modificar

```{r}
library(dplyr)
contrata4 <- contrata %>%
             arrange(SECTOR.DEL.PROYECTO, VALOR.CONTRATO, desc(VALOR.DEL.PROYECTO))
head(contrata4)
```


```{r}
rm(contrata1, contrata2, contrata3, contrata4)
```


### Resumir

```{r}
library(dplyr)
contrata1 <- contrata %>%
             summarise(mediaValorContrato = mean(VALOR.CONTRATO, na.rm = TRUE))
contrata1
```
```{r}
library(dplyr)
contrata2 <- contrata %>%
             summarise(mediaValorContrato = mean(VALOR.CONTRATO, na.rm = TRUE),
                       maximoValorContrato = max(VALOR.CONTRATO, na.rm = TRUE),
                       maximoValorProyecto = max(VALOR.DEL.PROYECTO, na.rm = TRUE))
contrata2
```

```{r}
rm(contrata1, contrata2)
```



### Agrupar

```{r}
library(dplyr)
contrata1 <- contrata %>%
             group_by(SECTOR.DEL.PROYECTO)
head(contrata1)

```


```{r}
library(dplyr)
contrata2 <- contrata %>%
             group_by(SECTOR.DEL.PROYECTO) %>%
             summarise(mediaValorContrato = mean(VALOR.CONTRATO, na.rm = TRUE))
head(contrata2)
```





## Algunas preguntas a la base de datos.

### ¿Cómo están distribuido los contratos en los 26 sectores?

```{r}
library(ggplot2)
ggplot(contrata, aes(SECTOR.DEL.PROYECTO)) +
  geom_bar() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```

```{r}
tabla1 <- sort(table(contrata$SECTOR.DEL.PROYECTO), decreasing = TRUE)
contrata$SECTOR.DEL.PROYECTO <- factor(contrata$SECTOR.DEL.PROYECTO,
                                       levels = names(tabla1))
library(ggplot2)
ggplot(contrata, aes(SECTOR.DEL.PROYECTO)) +
  geom_bar() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) 
```

### ¿Qué relación hay entre el sector del proyecto con el presupuesto del proyecto?

```{r}
library(dplyr)
proy_sect <- contrata %>%
           group_by(SECTOR.DEL.PROYECTO) %>%
           summarize(VALOR.DEL.PROYECTO.TOTAL = sum(VALOR.DEL.PROYECTO)) %>%
           dplyr::arrange(desc(VALOR.DEL.PROYECTO.TOTAL)) %>%
           mutate(SECTOR.DEL.PROYECTO = 
                    factor(SECTOR.DEL.PROYECTO, levels = SECTOR.DEL.PROYECTO))

proy_sect
```

```{r}
library(ggplot2)
ggplot(proy_sect, aes(SECTOR.DEL.PROYECTO, VALOR.DEL.PROYECTO.TOTAL)) +
     geom_bar(stat = "identity") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) 
```


```{r}
library(ggplot2)
ggplot(proy_sect, aes(SECTOR.DEL.PROYECTO, VALOR.DEL.PROYECTO.TOTAL)) +
     geom_bar(stat = "identity") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  scale_y_log10()
```

### ¿Cuáles fueron los 7 contratistas con mayor número de contratos?


```{r}
library(dplyr)
numcontra <- contrata %>%
             group_by(IDENTIFICACIÓN.CONTRATISTA, NOMBRE.CONTRATISTA) %>%
             summarise(num_contratos = n()) %>%
             arrange(desc(num_contratos))

numcontra %>%
  ungroup() %>%
  select(NOMBRE.CONTRATISTA, num_contratos) %>%
  slice(1:7)
```

