---
title: "Análisis Aglomerativo II"
author: "Universidad Nacional de Colombia, Sede Medellín"
date: "Jueves, 3 de noviembre de 2016"
output: 
  html_document: 
    toc: yes
---
...
```{r}
ejem1 <- read.csv2("ejemplo_simple.csv")

print(ejem1)

plot(ejem1, pch = 19, asp = 1)
with(ejem1, text(sp1, sp2, labels = rownames(ejem1), pos = 2))

ejem1.prop <- decostand(ejem1, "total")
ejem1.norm <- decostand(ejem1, "normalize")

ejem1.eu <- vegdist(ejem1.prop, "euc")
ejem1.ch <- vegdist(ejem1.norm, "euc")

plot(ejem1.prop, pch = 19, asp = 1, xlim = c(0, 1), ylim = c(0, 1))
with(ejem1.norm, points(sp1,sp2, pch = 19, col = "blue"))
with(ejem1.prop, text(sp1, sp2, labels = row.names(ejem1.prop), pos = 2))
with(ejem1.norm, text(sp1, sp2, labels = row.names(ejem1.norm), col = "red", pos = 4))

ejem1.eu.prop <- hclust(ejem1.eu, method = "complete")
ejem1.ch.norm <- hclust(ejem1.ch, method = "complete")

op <- par(no.readonly = TRUE)
par(mfrow = c(1, 2))
plot(ejem1.eu.prop, hang = -1)
plot(ejem1.ch.norm, hang = -1)
par(op)
```
...
# Cortar los dendrograms para comparar la membresía de lso grupos entre si.
