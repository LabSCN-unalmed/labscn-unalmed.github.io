...
- MÃºltiples tipos de unidades experimentales se almacenas en las misma tabla.
...
- *m*: GÃ©nero masculino.
- *f*: GÃ©nero femenino.
- *04*: Rango de edad 0-4 aÃ±os.
- *65*: Mayor de 65 aÃ±os.
- *u*: Desconocido.
...
require(plyr)
limpio <- arrange(limpio, pais, columna, aÃ±o)
head(limpio,20)
```
...
limpio <- limpio[c("pais", "aÃ±o", "sexo", "edad", "casos")]
...
g1 + geom_point() + facet_grid(edad ~ .) +
  ...
  ylab("log10 del NÃºmero de casos") + xlab("País")
